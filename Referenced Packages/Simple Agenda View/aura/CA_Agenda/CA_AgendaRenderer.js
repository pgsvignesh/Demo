({
	afterRender: function(component, helper) {
        return;
        var oSvg = component.find("svg_content");
        if (oSvg.length == undefined) {
            oSvg = [oSvg];
        }
        for (var nJ = 0; nJ < oSvg.length; nJ++) {
            var oEl = oSvg[nJ].getElement(),
                sHTML;
                console.log('-- ' , oEl);
            if (oEl == null) {
                continue;
            ...
            sHTML = oEl.innerText 
            	|| (oEl.firstChild && oEl.firstChild.data ? oEl.firstChild.data : "");
            
            oEl.innerHTML = sHTML.replace("<![CDATA[", "").replace("]]>", "");        
            oEl.style.display = 'inline-block';
        }
        console.log('SVG changing here ');
